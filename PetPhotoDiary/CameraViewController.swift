...
class CameraViewController: SwiftyCamViewController, SwiftyCamViewControllerDelegate, AVAudioPlayerDelegate  {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        photoViewController.photoImage = self.imagePhoto
    }
    
    func getCameraSound(path: String) -> Dictionary<String, Any> {
        var returnValue: Dictionary<String, Any> = [:]
        for item in GlobalData.sharedInstance.cameraSoundList {
    ...
    func playCameraSound() -> Bool {
        var ret = false
        let item = getCameraSound(path: GlobalData.sharedInstance.cameraSoundSelected)
        if let filepath = item["path"] as? String,
           let id = item["id"] as? Int {
            if id < GlobalData.sharedInstance.cameraSoundPrefixCount {
                ...
                ret = self.playSound(path: stringPath)
            }
            else {
    ...
    func swiftyCam(_ swiftyCam: SwiftyCamViewController, willTake camera: SwiftyCamViewController.CameraSelection) {
        print("Will take photo")
        if self.playCameraSound() == true {
            sleep(Constants.CAMERA_SOUND_DELAY)
        }
    }
    ...
    func swiftyCam(_ swiftyCam: SwiftyCamViewController, didTake photo: UIImage) {
        ...
        self.imagePhoto = photo
        performSegue(withIdentifier: "goPreviewPhoto", sender: self)
    }
